import { Component, AfterViewInit, OnChanges, ViewChild, ElementRef, Input, NgZone, Inject, ChangeDetectorRef } from '@angular/core';
...
export class EllipsisComponent implements AfterViewInit, OnChanges {
  ...
  private genType() {
    ...
    this.cls = {
      'ml-ellipsis': true,
      'ml-ellipsis__lines': lines && !isSupportLineClamp,
      'ml-ellipsis__line-clamp': lines && isSupportLineClamp,
    };
    if (!lines && !length) {
      this.type = 'default';
    } else if (!lines) {
  ...
  private gen() {
    ...
    if (type === 'length') {
      ...
      this.getEl('.ml-ellipsis__handle').style.height = `${targetHeight}px`;
  ...
  refresh(): void {
    ...
    this.executeOnStable(() => {
      ...
      this.gen();
      if (type !== 'line') {
        const el = this.getEl('.ml-ellipsis');
        if (el) {
          el.innerHTML = html;
        }
      }
